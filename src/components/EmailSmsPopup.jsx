  ...
  const submit = async (ev) => {
    ...
    try {
      if (supabase) {
        ...
        await fetch('/.netlify/functions/subscribe-marketing', {
          method: 'POST',
        ...
        <>
          ...
          >
            <div className="relative rounded-t-3xl sm:rounded-3xl border border-white/10 bg-onyx-900 p-7 shadow-lift">
              <button onClick={dismiss} className="absolute right-4 top-4 flex h-8 w-8 items-center justify-center rounded-full border border-white/10 text-bone/50 hover:text-bone">
                <X size={15} />
              </button>
              ...
              {status === 'done' ? (
                <div className="flex flex-col items-center gap-3 py-4 text-center">
                  ...
                  <p className="font-display text-xl text-bone">You're on the list.</p>
                ...
                <>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-display text-lg text-bone">Stay in the know.</p>
                      <p className="text-xs text-bone/50">Deals, product drops, and member perks.</p>
                    </div>
                  </div>

                  <form onSubmit={submit} className="mt-5 space-y-3">
                    {errors.all && <p className="text-xs text-red-400">{errors.all}</p>}
                    <div>
                      <input
                        type="email"
                    ...
                    <p className="text-center text-xs text-bone/30">No spam. Unsubscribe anytime.</p>
                  </form>
                </>
              )}
            </div>
